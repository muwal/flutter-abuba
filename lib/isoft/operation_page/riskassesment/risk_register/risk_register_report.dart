            ...
            children: <Widget>[
              StreamBuilder(
                stream: showDataFiltered
                    ? Firestore.instance.collection('risk_assesment2').where('createdTime', isGreaterThanOrEqualTo: dateStart).orderBy('createdTime', descending: true).snapshots()
                    : Firestore.instance.collection('risk_assesment2').orderBy('createdTime', descending: true).snapshots(),
                builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                  ...
                  return Column(
                    children: List.generate(snapshot.data.documents.length, (index) {
                      Timestamp tanggalStart = snapshot.data.documents[index].data['createdTime'];
                        ...
                        if (tanggalBantu.isAfter(dateEnd.subtract(Duration(days: -1))) == true) {
                          ...
                          return ListTile(
                            leading: Container(
                              ...
                              child: Row(
                                children: <Widget>[
                                  Flexible(
                                    child: Text(
                                      'Risk Assesment No. RA-${snapshot.data.documents[index].data['maxid'].toString().padLeft(4, '0')}',
                                      style: TextStyle(
                            ...
                            trailing: GestureDetector(
                              onTap: () async {
                                /*Firestore.instance.collection('risk_assesment2').document(snapshot.data.documents[index].documentID).snapshots().listen((data) {
                                  setState(() {
                                    no = data.data['jenis'];
                                  });
                                });*/
                                ...
                                Navigator.push(
                                    context,
                                    MyCustomRoute(
                                        builder: (context) => DetailReport(idUser: widget.idUser, namaUser: widget.namaUser, departmentUser: widget.departmentUser, index: snapshot.data.documents[index].data['createdTime'], noData: no)));
                              },
                              child: Container(
                                alignment: Alignment.centerRight,
