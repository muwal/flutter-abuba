...
class CarIA extends StatefulWidget {
  final int idUser;
  final String namaUser;
  final String departmentUser;
  CarIA({this.idUser, this.namaUser, this.departmentUser});

  @override
  _CarIAState createState() => _CarIAState();
}

class _CarIAState extends State<CarIA> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    reference.where('category', isEqualTo: 1).snapshots().listen((data) {
                      ...
                      if (showDataFiltered && dateEnd != null && dateStart != null) {
                        if (tanggalBantu.isAfter(dateEnd.subtract(Duration(days: -1))) == true) {
                          ...
                          if (snapshot.data.documents[index].data['userDituju'] == widget.idUser) {
                            if (snapshot.data.documents[index].data['category'] == 1) {
                              return ListTile(
                                leading: Container(
                                  padding: EdgeInsets.only(top: 5.0),
                                  alignment: Alignment.centerRight,
