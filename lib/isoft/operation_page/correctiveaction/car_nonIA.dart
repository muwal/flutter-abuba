...
import 'package:flutter_abuba/isoft/operation_page/correctiveaction/form_createLanjut.dart';
import 'package:intl/intl.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

class CarNonIA extends StatefulWidget {
  final int idUser;
  final String namaUser;
  final String departmentUser;
  CarNonIA({this.idUser, this.namaUser, this.departmentUser});

  @override
  _CarNonIAState createState() => _CarNonIAState();
}

class _CarNonIAState extends State<CarNonIA> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    reference.where('category', isEqualTo: 2).snapshots().listen((data) {
      data.documentChanges.forEach((change) {
        setState(() {
          ...
          userTuju.add(change.document.data['userDituju']);
                ...
                builder: (BuildContext context,
                  ...
                  return Column(
                    children: List.generate(snapshot.data.documents.length, (index) {
                      ...
                      Timestamp tanggalBuat = snapshot.data.documents[index].data['dateCreated'];

                      Firestore.instance.collection('user').where('id',isEqualTo: snapshot.data.documents[index].data['userDituju']).snapshots().listen((data3) {
                      ...
                      if (showDataFiltered && dateEnd != null && dateStart != null) {
                        if (tanggalBantu.isAfter(dateEnd.subtract(Duration(days: -1))) == true) {
                          ...
                          if (snapshot.data.documents[index].data['userDituju'] == widget.idUser) {
                            if (snapshot.data.documents[index].data['category'] == 2) {
                              return ListTile(
                                leading: Container(
                                  padding: EdgeInsets.only(top: 5.0),
                                  alignment: Alignment.centerRight,
                                  ...
                                  child: Row(
                                    children: <Widget>[
                                      Flexible(
                                        child: Text(
                                          tanggalBuat.toDate().toString().substring(8, 10) + '/' + tanggalBuat.toDate().toString().substring(5, 7) + '/' + tanggalBuat.toDate().toString().substring(0, 4),
                                          style: TextStyle(
                                            fontSize: 12.0,
                                      ...
                                      children: <Widget>[
                                        Flexible(
                                          child: Text(
                                            ...
                                            style: TextStyle(
                                                fontSize: 12.0,
                                                color: snapshot.data.documents[index].data['status'] == 'OPEN'
                                                    ? Colors.green
                                                    : snapshot.data.documents[index].data['status'] == 'ONGOING'
                                                        ? Colors.orangeAccent
                                                        : snapshot.data.documents[index].data['status'] == 'DONE'
