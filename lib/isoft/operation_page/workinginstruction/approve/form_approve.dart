...
class FormApproveWorking extends StatefulWidget {
  final int idUser;
  final String namaUser;
  final String departmentUser;
  FormApproveWorking({this.idUser, this.namaUser, this.departmentUser});

  @override
  _FormApproveWorkingState createState() => _FormApproveWorkingState();
}

class _FormApproveWorkingState extends State<FormApproveWorking> with TickerProviderStateMixin {
    ...
    reference.where('approvedBy', isEqualTo: widget.idUser).where('status', isEqualTo: 'REVIEWED').snapshots().listen((querySnapshot) {
      querySnapshot.documentChanges.forEach((change) {
        setState(() {
          ...
          verifyBy.add(change.document.data['approvedBy']);
                                      ...
                                      ButtonTheme(
                                        ...
                                        child: OutlineButton(
                                            ...
                                            onPressed: () {
                                              Navigator.push(context,
                                                  MyCustomRoute(
                                                      builder: (context) => new FormDetailApproveWorking(idUser: widget.idUser, namaUser: widget.namaUser, departmentUser: widget.departmentUser, index: docIDList[index], pJabatan: pJabatanList[index], pDocTerkait: pDocTerkaitList[index], pDepartment: pDepartmentList[index], pComment: pCommentList[index], pCommentStatus: pCommentStatusList[index], tujuanCommentCreated: tujuanCommentCreatedList[index], ruangLingkupCommentCreated: ruangLingkupCommentCreatedList[index], referensiCommentCreated: referensiCommentCreatedList[index], istilahCommentCreated: istilahCommentCreatedList[index], prosedurCommentTgl: prosedurCommentTglList[index], risikoCommentCreated: risikoCommentCreatedList[index], bantureferensi: bantureferensiList[index], banturisiko: banturisikoList[index], banturuanglingkup: banturuanglingkupList[index], bantutujuan: bantutujuanList[index])
                                                  )
                                              );
                                            }
                                        ),
                      ...
                      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                        ...
                        return Column(
                          children: List.generate(snapshot.data.documents.length, (index) {
                            if (snapshot.data.documents[index].data['approvedBy'] == widget.idUser) {
                              if (snapshot.data.documents[index].data['status'] == 'REVIEWED') {
                                return Container(
                                  color: Colors.white,
                                  child: ExpansionTile(
                                    title: Column(
