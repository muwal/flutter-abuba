...
class DetailWaktu extends StatefulWidget {
  ...
  DetailWaktu({this.idUser, this.namaUser, this.departmentUser, this.idSupplier});
  @override
  _DetailWaktuState createState() => _DetailWaktuState();
}

class _DetailWaktuState extends State<DetailWaktu> with TickerProviderStateMixin {
    ...
    reference.where('id_supplier', isEqualTo: int.tryParse(widget.idSupplier.toString())).snapshots().listen((data2) {
      data2.documentChanges.forEach((change) {
        setState(() {
          ...
          incoming.add(change.document.data['jawaban_supplier_evaluation'][0]);

          if (change.document.data['jawaban_supplier_evaluation'][0] == 'NO') {
