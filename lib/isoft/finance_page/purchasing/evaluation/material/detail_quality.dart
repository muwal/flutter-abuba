...
class DetailQuality extends StatefulWidget {
  ...
  DetailQuality({this.idUser, this.namaUser, this.departmentUser, this.idSupplier});
  @override
  _DetailQualityState createState() => _DetailQualityState();
}

class _DetailQualityState extends State<DetailQuality> with TickerProviderStateMixin {
    ...
    reference.where('id_supplier', isEqualTo: int.tryParse(widget.idSupplier.toString())).snapshots().listen((data2) {
      data2.documentChanges.forEach((change) {
        setState(() {
          ...
          incoming.add(change.document.data['status']);
        });
      });
    });

    CollectionReference reference2 = Firestore.instance.collection('incoming');
    reference2.where('id_supplier', isEqualTo: int.tryParse(widget.idSupplier.toString())).where('status', isEqualTo: 'REJECTED').snapshots().listen((data3) {
      data3.documentChanges.forEach((change2) {
        setState(() {
          ...
          Firestore.instance.collection('material').where('id', isEqualTo: change2.document.data['id_material']).snapshots().listen((data4) {
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                              ...
                              children: <Widget>[
                                Flexible(
                                  child: Text(
                                    incoming.length == 0
                                      ? '0%'
                                      : (incoming.where((c) => c == 'APPROVED').toList().length / incoming.length * 100).round() > 100 
                                          ? '100%'
                                          : '${(incoming.where((c) => c == 'APPROVED').toList().length / incoming.length * 100).round().toString()}%',
                                    style: TextStyle(
                            ...
                            LinearPercentIndicator(
                              ...
                              animation: true,
                              lineHeight: 30.0,
                              percent: incoming.length == 0 ? 0 : incoming.where((c) => c == 'APPROVED').toList().length / incoming.length,
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            ...
                            Flexible(
                              child: Text(
                                incoming.where((c) => c == 'REJECTED').toList().length.toString(),
                                style: TextStyle(
                                    fontSize: 13.0,
                                    color: Colors.black54,
                                    fontWeight: FontWeight.w600),
