...
class FormMaintenance extends StatefulWidget {
  ...
  int count;
  FormMaintenance({this.idUser, this.namaUser, this.departmentUser, this.count});

  _FormMaintenanceState createState() => _FormMaintenanceState();
}

class _FormMaintenanceState extends State<FormMaintenance> with TickerProviderStateMixin {
  String barcode;
  ...
  void initState() {
    ...
    CollectionReference reference2 = Firestore.instance.collection('maintenance');
                  ...
                  children: <Widget>[
                    ListView(
                      ...
                      children: <Widget>[
                        ...
                        StreamBuilder(
                            stream: Firestore.instance.collection('maintenance').snapshots(),
                                                                              ...
                                                                              builder: (context) {
                                                                                return Checkbox2(
                                                                                  ...
                                                                                  onResult: (finalResult) {
                                                                                    DocumentReference docReference = Firestore.instance.collection('maintenance').document(snapshot.data.documents[index].documentID);
                                                          ...
                                                          child: Column(
                                                            children: <Widget>[
                                                              ...
                                                              Padding(
                                                                  padding: const EdgeInsets.only(left: 12.0, right: 12.0, bottom: 5.0, top: 10.0),
                                                                  child: Column(
                                                                      children: List.generate(jobSkip.length, (index) {
                                                                        return Container(
                                                                            padding: EdgeInsets.symmetric(vertical: 3.0),
                                                                            alignment: Alignment.topLeft,
                                                                            width: size,
                                                                            child: Row(
                                                                              children: <Widget>[
                                                                                RichText(
                                                                                  ...
                                                                                  overflow: TextOverflow.visible,
                                                                                  text: TextSpan(
                                                                                    text: '${(index + 1).toString()}. ${jobSkip[index]}',
                                                                                    style: TextStyle(
                                                                                        fontSize: 12.0, color: Colors.black87),
                                                                                  ),
                                                                                ),
                                                                                ...
                                                                                Tooltip(
                                                                                  child: jobTimeSkip[index] == null ? Icon(Icons.remove_circle, color: Colors.redAccent, size: 14.0) : Icon(Icons.check_circle, color: AbubaPallate.greenabuba, size: 14.0),
                                                                                  message: jobTimeSkip[index] == null ? 'Skipped' : 'Done',
                                                                                )
                                                                              ],
                                                                            )
                                                                        );
...
class _CheckboxState extends State<Checkbox> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    for (int x = 0; x < widget.jobdesc.length; x++) {
      Firestore.instance.collection('maintenance').document(widget.indexx).snapshots().listen((data) {
        setState(() {
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      if (value == 0) {
                                        ...
                                        showAlert = false;
                                        DocumentReference docReference = Firestore.instance.collection('maintenance').document(widget.indexx);
                                        ...
                                        docReference.updateData({
                                          ...
                                          setState(() {
                                            isProcess = false;
                                            if (_FormMaintenanceState().count > 0) {
                                              _FormMaintenanceState().count = _FormMaintenanceState().count - 1;
                                            } else {
                                              _FormMaintenanceState().count = 0;
                                            }
                                          });
                                          print('success');
                                        }).catchError((error) {
